...
async def test_stalled_assigned_ride_is_redispatched_to_another_driver(db_session, rider_auth, driver_auth, marketplace_objects):
    ...
    await db_session.execute(
        ...
        {"id": "00000000-0000-0000-0000-000000000301", "email": "candidate-redispatch@test.local"},
    )
    ...
    ride = Ride(
        ...
        dropoff_latitude=2,
        dropoff_longitude=2,
        requested_at=datetime.now(timezone.utc) - timedelta(minutes=12),
    ...
    accepted_offer = DriverOffer(
        ...
        offered_at=datetime.now(timezone.utc) - timedelta(minutes=11),
        responded_at=datetime.now(timezone.utc) - timedelta(minutes=10),
    ...
    await dispatch_service.auto_redispatch_stalled_prepickup_rides(db_session)
...
async def test_driver_profile_includes_reassignment_notice_after_auto_redispatch(db_session, rider_auth, driver_auth, marketplace_objects):
    ride_service = marketplace_objects["ride_service"]
    driver_service = marketplace_objects["driver_service"]
    Ride = marketplace_objects["Ride"]
    dispatch_service = marketplace_objects["dispatch_service"]
    ...
    profile = await driver_service.get_profile(db_session, driver_auth["user_id"])
    assert profile.reassigned_ride_id == ride.id
    assert profile.reassignment_notice is not None
